...
- `empty` reward
  ...
  - ver2. 한 에피소드 당, 만약 이미 터진 곳의 옆자리(누른 곳을 중심으로 한 3\*3 행렬) 가 이미 터져있을 때, 기계는 지적 추론을 한 것이므로 더 큰 보상을 주도록 하였다. 예를 들어, 아래의 사진에서 빨간 점을 누른 다고 했을 때, 빨간 점을 중심으로 한 3*3 gird_world 를 탐색한다. 이 때, 이미 탐색 된 것(아래의 그림에서는 (2,3)) 이 있다면 3의 보상을 주었다. <br>
